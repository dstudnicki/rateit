...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (type === "post") {
            ...
            const result = await Post.updateOne({ "comments._id": id }, { $pull: { comments: { _id: id } } });
            ...
            const result = await Company.updateOne({ "reviews._id": id }, { $pull: { reviews: { _id: id } } });
            ...
            if (company && company.reviews) {
                ...
                await Company.updateOne({ _id: company._id }, { $set: { averageRating } });
            }
        } else {
